...
class TestPassiveScalarBasics:
    ...
    def test_scalar_evolver_creation(self):
        ...
        evolver = PassiveScalarEvolver(
            self.grid, kappa=0.02, source_fn=source, name="tracer"
        )
...
class TestSolverIntegration:
    ...
    def test_solver_with_scalars(self):
        ...
        passive_scalars = {
            ...
            'temperature': {
                'kappa': 0.02,
                'source': LocalizedSource(1.0, np.pi, np.pi, 0.5)
            }
        }
    ...
    def test_diagnostics_with_scalars(self):
        ...
        solver = gSQGSolverWithScalars(
            self.grid,
            alpha=1.0,
            passive_scalars={'dye': {'kappa': 0.01}}
        )
        
        state = solver.initialize(
            seed=42,
            scalar_init={'dye': np.ones((self.N, self.N))}
        )
