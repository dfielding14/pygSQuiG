...
from pygsquig.io.config import (
    ScalarSourceConfig, PassiveScalarConfig, ScalarsConfig,
    RunConfig, GridConfig, SolverConfig
)
...
class TestPassiveScalarConfig:
    ...
    def test_scalar_with_source(self):
        ...
        source = ScalarSourceConfig(
            type="localized",
            parameters={"x0": 3.14, "y0": 3.14, "sigma": 0.5, "amplitude": 1.0}
        )
        scalar = PassiveScalarConfig(
            name="temperature",
            kappa=0.02,
...
class TestScalarConfigIntegration:
    ...
    def test_runconfig_with_scalars(self):
        ...
        scalars = ScalarsConfig(
            enabled=True,
            species=[
                PassiveScalarConfig(name="tracer", kappa=0.01)
            ]
        )
        config = RunConfig(
            grid=GridConfig(N=128),
            solver=SolverConfig(alpha=1.0),
            scalars=scalars
        )
            ...
            scalars=ScalarsConfig(
                ...
                species=[
                    PassiveScalarConfig(
                        ...
                        source=ScalarSourceConfig(
                            type="localized",
                            parameters={"x0": 3.14, "y0": 3.14, "sigma": 0.5}
                        ),
                        initial_condition="gaussian",
