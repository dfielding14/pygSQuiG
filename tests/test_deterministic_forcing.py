...
class TestShearLayerForcing:
    ...
    def test_linear_shear(self):
        ...
        forcing = ShearLayerForcing(amplitude=1.0, profile="linear", direction="y")
...
class TestTimeModulatedForcing:
    ...
    def test_sine_modulation(self):
        ...
        modulated = TimeModulatedForcing(
            base_forcing=base, modulation="sine", frequency=1.0, phase=0.0
        )
    ...
    def test_exponential_decay(self):
        ...
        modulated = TimeModulatedForcing(
            base_forcing=base, modulation="exponential", decay_rate=2.0
        )
    ...
    def test_factory_oscillating(self):
        ...
        forcing = make_oscillating_forcing(
            base_pattern="taylor_green", frequency=2.0, amplitude=3.0, k=4
        )
...
class TestCombinedForcing:
    ...
    def test_linear_combination(self):
        ...
        combined = CombinedDeterministicForcing([(1.0, f1), (0.5, f2)])
...
class TestForcingDiagnostics:
    ...
    def test_energy_injection(self):
        ...
        theta = jnp.sin(2 * 2 * np.pi * self.grid.x / self.L) * jnp.cos(
            2 * 2 * np.pi * self.grid.y / self.L
        )
...
class TestSolverIntegration:
    ...
    def test_deterministic_forcing_in_solver(self):
        ...
        # Create solver
        solver = gSQGSolver(grid=self.grid, alpha=1.0, nu_p=1e-4, p=8)
