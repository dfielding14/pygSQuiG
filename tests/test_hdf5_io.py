...
class TestOutputIO:
    ...
    def test_save_load_physical_fields(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            # Save output
            save_output(data, grid, time=10.0, metadata=metadata, 
                       filename=output_path, compress=True)
    ...
    def test_save_spectral_fields(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            save_output(data, grid, time=5.0, metadata={}, 
...
class TestDiagnosticsIO:
    ...
    def test_save_load_array_diagnostics(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            spectrum1 = k**(-5/3)
            diag1 = {
                "energy_spectrum": spectrum1,
                "total_energy": np.sum(spectrum1)
            }
            save_diagnostics(diag1, time=0.0, filename=diag_path, mode="write")
            ...
            spectrum2 = 1.1 * k**(-5/3)
            diag2 = {
                "energy_spectrum": spectrum2,
                "total_energy": np.sum(spectrum2)
            }
            save_diagnostics(diag2, time=1.0, filename=diag_path, mode="append")
