...
class AdaptivegSQGSolver:
    ...
    def __init__(
        ...
        self.current_dt = estimate_initial_timestep(
            grid, nu_p, p,
            target_cfl=cfl_config.target_cfl if cfl_config else 0.5
        )
    ...
    def step(
        ...
        **kwargs
        ...
        dt, dt_diags = self.timestepper.compute_timestep(
            state, u, v, self.nu_p, self.p
        )
        ...
        try:
            ...
            # Check stability
            is_stable, reason = self.timestepper.check_stability(
                state, new_state, dt
            )
        ...
        **kwargs
        ...
        while current_state['time'] < t_final:
            ...
            if self.verbose and step_count % 100 == 0:
                ...
                print(f"  Step {step_count}: t={current_state['time']:.3f} "
    ...
    def get_diagnostics(self, state: State) -> Dict[str, float]:
        ...
        _, dt_diags = self.timestepper.compute_timestep(
            state, u, v, self.nu_p, self.p
        )
        
        diags.update({
            f"cfl_{key}": value
            for key, value in dt_diags.items()
