...
class MultiGPUSolver:
    ...
    def _setup_domain_decomposition(self):
        ...
        if N % n_dev != 0:
            raise ConfigurationError(
                f"Grid size {N} must be divisible by device count {n_dev}"
            )
    ...
    def _setup_data_parallel(self):
        ...
        @functools.partial(
            jax.pmap,
            axis_name="batch",
            static_broadcasted_argnums=(2,)  # dt is static
        )
    ...
    def benchmark_scaling(self, n_steps: int = 100, dt: float = 0.001) -> Dict[str, float]:
        ...
        if self.config.n_devices > 1:
            ...
            single_solver = MultiGPUSolver(
                self.grid, self.alpha, self.nu_p, self.p,
                ParallelConfig(n_devices=1)
            )
            ...
            for _ in range(n_steps):
                theta_hat = single_solver.ensemble_step(theta_hat[None, ...], dt, key[None, ...])[0]
        ...
        if self.config.ensemble_size:
            ...
            theta_hat_batch = jax.vmap(lambda k: fft2(jax.random.normal(k, (self.grid.N, self.grid.N))))(keys)
...
def optimize_domain_decomposition(
    ...
    if decomp_type == "1d":
        ...
        if N % px != 0 or N % py != 0:
            raise ConfigurationError(
                f"Grid size {N} incompatible with {px}x{py} decomposition"
            )
...
def ensemble_statistics(
    ...
    spectra = jax.vmap(
        lambda state: compute_energy_spectrum(state, grid, alpha)
    )(states_array)
    ...
    # Energy statistics
    energies = jax.vmap(
        lambda state: compute_total_energy(state, grid, alpha)
    )(states_array)
