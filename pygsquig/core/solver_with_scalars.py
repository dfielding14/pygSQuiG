...
class gSQGSolverWithScalars(gSQGSolver):
    ...
    def step(
        ...
        **kwargs
        ...
        # Step active scalar
        new_base_state = super().step(
            extended_state.base_state, dt, forcing, damping, **kwargs
        )
        ...
        if self.scalar_evolver is not None and extended_state.scalar_state is not None:
            ...
            # Step all scalars
            new_scalar_state = self.scalar_evolver.step(
                extended_state.scalar_state, dt, u, v
            )
        else:
    ...
    def get_diagnostics(
        ...
        if (isinstance(state, ExtendedState) and 
            state.scalar_state is not None and
            self.scalar_evolver is not None):
    ...
    def add_passive_scalar(
        ...
        if self.scalar_evolver is None:
            ...
            self.scalar_evolver = MultiSpeciesEvolver(
                grid=self.grid,
                species={name: {'kappa': kappa, 'source': source}}
            )
        else:
            # Add to existing evolver
            if name in self.scalar_evolver.evolvers:
                raise ConfigurationError(
                    f"Scalar '{name}' already exists"
                )
