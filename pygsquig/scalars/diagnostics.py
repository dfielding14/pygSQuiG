...
def compute_scalar_dissipation(scalar_hat: jnp.ndarray, grid: Grid, kappa: float) -> float:
...
def compute_mixing_efficiency(
    scalar_hat_initial: jnp.ndarray, scalar_hat_final: jnp.ndarray, grid: Grid, time_elapsed: float
) -> float:
...
def compute_scalar_pdf_moments(scalar_hat: jnp.ndarray, max_moment: int = 4) -> dict:
...
def compute_batchelor_scale(
    scalar_hat: jnp.ndarray, grid: Grid, kappa: float, epsilon: float
) -> float:
...
def compute_peclet_number(u_rms: float, L_integral: float, kappa: float) -> float:
