...
class ExponentialGrowth(SourceTerm):
    ...
    def __post_init__(self):
        ...
        if not isinstance(self.rate, (float, int)):
            raise SourceTermError(
                f"Rate must be numeric, got {type(self.rate).__name__}"
            )
...
class LocalizedSource(SourceTerm):
    ...
    @staticmethod
    @jax.jit
    def _compute_gaussian_jit(x: jnp.ndarray, y: jnp.ndarray, 
                             x0: float, y0: float, sigma2: float,
        ...
        dx = jnp.minimum(
            jnp.abs(x - x0),
            jnp.abs(x - x0 + 2*jnp.pi)
        )
        dx = jnp.minimum(dx, jnp.abs(x - x0 - 2*jnp.pi))
        
        dy = jnp.minimum(
            jnp.abs(y - y0),
            jnp.abs(y - y0 + 2*jnp.pi)
        )
    ...
    def _compute_gaussian(self, x: jnp.ndarray, y: jnp.ndarray, 
                         amplitude: float) -> jnp.ndarray:
        """Compute Gaussian using JIT-compiled function."""
        return self._compute_gaussian_jit(x, y, self.x0, self.y0, 
                                         self.sigma2, amplitude)
...
class ChemicalReaction(SourceTerm):
    ...
    def __post_init__(self):
        ...
        if self.rate < 0:
            raise SourceTermError(
                f"Reaction rate must be non-negative, got {self.rate}"
            )
    ...
    def __init__(self, sources: list):
        ...
        for weight, source in sources:
            if not isinstance(source, SourceTerm):
                raise SourceTermError(
                    f"Expected SourceTerm, got {type(source).__name__}"
                )
...
def make_heating_source(x0: float, y0: float, radius: float = 0.5,
                       power: float = 1.0) -> LocalizedSource:
    ...
    return LocalizedSource(
        ...
        x0=x0,
        y0=y0,
        sigma=radius / 2.0  # 2-sigma ~ radius
    )
...
def make_reaction_source(reaction_rate: float, 
                        threshold: float = 0.0) -> ChemicalReaction:
