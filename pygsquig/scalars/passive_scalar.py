...
@jax.jit
def compute_scalar_advection(scalar: jnp.ndarray, u: jnp.ndarray, 
                            v: jnp.ndarray, grid: Grid) -> jnp.ndarray:
...
@jax.jit
def compute_scalar_diffusion(scalar_hat: jnp.ndarray, grid: Grid,
...
class PassiveScalarEvolver:
    ...
    def __post_init__(self):
        ...
        if self.source_fn is not None and not isinstance(self.source_fn, SourceTerm):
            raise PassiveScalarError(
                f"source_fn must be a SourceTerm instance, "
                f"got {type(self.source_fn).__name__}"
            )
    
    def initialize(self, scalar0: Optional[jnp.ndarray] = None,
                  seed: Optional[int] = None) -> PassiveScalarState:
        ...
        if scalar0 is not None:
            ...
            raise PassiveScalarError(
                "Must provide either scalar0 or seed for initialization"
            )
    ...
    def compute_rhs(self, scalar_hat: jnp.ndarray, u: jnp.ndarray,
                   v: jnp.ndarray, t: float = 0.0) -> jnp.ndarray:
        ...
        return compute_passive_scalar_rhs(
            scalar_hat, u, v, self.grid, self.kappa, source_hat
        )
    
    def step(self, state: PassiveScalarState, dt: float,
            u: jnp.ndarray, v: jnp.ndarray) -> PassiveScalarState:
...
class MultiSpeciesEvolver:
        ...
        for name, evolver in self.evolvers.items():
            if name in initial_fields:
                ...
                state = evolver.initialize(
                    scalar0=jnp.zeros((self.grid.N, self.grid.N))
                )
            scalar_states[name] = state.scalar_hat
            
        return MultiScalarState(
            scalars=scalar_states,
            time=0.0,
            diagnostics={}
        )
    
    def step(self, state: MultiScalarState, dt: float,
            u: jnp.ndarray, v: jnp.ndarray) -> MultiScalarState:
        ...
        for name, evolver in self.evolvers.items():
            ...
            scalar_state = PassiveScalarState(
        ...
        if self.coupled_sources:
            new_scalars = self._apply_coupled_sources(
                new_scalars, state.time + dt, dt
            )
    ...
    def _apply_coupled_sources(self, scalars: Dict[str, jnp.ndarray],
                              t: float, dt: float) -> Dict[str, jnp.ndarray]:
