...
class RunConfigAdapter:
    ...
    def _get_forcing_adapter(self):
        ...
        if (self._config.get('forcing.enabled', False) or 
            self._config.get('forcing.epsilon', 0) > 0 or
            self._config.get('forcing.type') == 'ring' or
            self._config.get('forcing.kf') is not None):
...
class SolverConfigAdapter:
    ...
    def __init__(self, config: Config):
        ...
        self.dissipation = type('DissipationConfig', (), {
        ...
        if config.get('damping.enabled', False) or config.get('damping.mu', 0) > 0:
            ...
            self.damping = type('DampingConfig', (), {
                'mu': config.get('damping.mu'),
                'kf': forcing_kf * factor,
                'type': 'linear_drag'
            })()
        else:
            self.damping = None
        ...
        self.time_integration = type('TimeIntegrationConfig', (), {
