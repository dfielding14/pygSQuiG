...
class Config:
    ...
    def __init__(self, config_dict: Optional[Dict[str, Any]] = None):
        ...
        if ('forcing' in (config_dict or {}) and 
            not ('enabled' in (config_dict or {}).get('forcing', {}))):
            # If forcing params provided but enabled not specified, auto-enable
            if (self._config['forcing'].get('epsilon', 0) > 0 or
                self._config['forcing'].get('type') == 'ring'):
                self._config['forcing']['enabled'] = True
        
        # Auto-enable damping if damping parameters are provided
        if ('damping' in (config_dict or {}) and
            not ('enabled' in (config_dict or {}).get('damping', {}))):
