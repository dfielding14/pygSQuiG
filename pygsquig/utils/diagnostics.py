...
def _compute_energy_density(theta_hat: jnp.ndarray, k2: jnp.ndarray, alpha: float) -> jnp.ndarray:
...
@partial(jax.jit, static_argnums=(2,))
def compute_enstrophy(theta_hat: jnp.ndarray, grid: Grid, alpha: float) -> jnp.ndarray:
...
def compute_energy_flux(
    theta_hat: jnp.ndarray, forcing_hat: Optional[jnp.ndarray], grid: Grid
) -> jnp.ndarray:
...
@partial(jax.jit, static_argnums=(2,))
def compute_total_energy(theta_hat: jnp.ndarray, grid: Grid, alpha: float) -> jnp.ndarray:
...
@partial(jax.jit, static_argnums=(2,))
def compute_palinstrophy(theta_hat: jnp.ndarray, grid: Grid, alpha: float) -> jnp.ndarray:
