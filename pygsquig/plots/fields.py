...
def plot_diagnostic_summary(
    ...
    q_hat = fractional_laplacian(theta_hat, grid, alpha)
    q = ifft2(q_hat)
    plot_field_slice(q, grid, "q", ax=ax2, show_colorbar=False,
    ...
    plot_field_slice(speed, grid, "|u|", ax=ax3, show_colorbar=False,
    ...
    if forcing_info and 'kf' in forcing_info:
        ax4.axvline(forcing_info['kf'], color='green', linestyle=':', 
                    alpha=0.7, label='Forcing')
    ...
    text = '\n'.join(text_lines)
    ax5.text(0.5, 0.5, text, transform=ax5.transAxes,
