...
def plot_energy_spectrum_with_analysis(
    ...
    # Main spectrum
    ax.loglog(k[1:], E_k[1:], PlotStyle.SPECTRUM_COLOR, 
              linewidth=2.5, label='E(k)')
    ...
    if len(k) > 20:
        ...
        if len(k_ref) > 10:
            for i, (label, slope) in enumerate(reference_slopes.items()):
                ...
                ax.loglog(k_ref, E_ref, '--', linewidth=1.5, 
    ...
    if inertial_range:
        ...
        if len(k_inertial) > 2:
            ...
            ax.loglog(k_inertial, E_fit, 'k:', linewidth=2,
                      label=f'Fit: $k^{{{measured_slope:.2f}}}$')
            
            # Shade inertial range
            ax.axvspan(k_min, k_max, alpha=0.1, color='gray',
...
def plot_spectrum_evolution(
    ...
    for i, t in enumerate(times):
        ...
        ax.loglog(k[1:], E_k[1:], color=colors[i], 
                  linewidth=2, label=f't={t:.1f}')
    
    # Add reference slope
    k_ref = k[20:60]
    E_ref = E_k[30] * (k_ref / k_ref[10])**(-5/3)
    ax.loglog(k_ref, E_ref, 'k--', alpha=0.5, 
...
def plot_compensated_spectrum(
    ...
    ax.semilogx(k[1:], E_comp[1:], PlotStyle.SPECTRUM_COLOR, 
                linewidth=2.5)
    ...
    ax.axhline(np.median(E_comp[20:60]), color='r', linestyle='--', 
               alpha=0.5, label='Expected plateau')
    ax.legend()
