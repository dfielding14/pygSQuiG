    ...
    def wrapper(*args, **kwargs):
        ...
        if not -2 <= alpha <= 2:
            raise ValidationError(
                f"alpha must be in [-2, 2], got {alpha}"
            )
...
def validate_grid_size(func: Callable) -> Callable:
    ...
    def wrapper(N: int, *args, **kwargs):
        if not isinstance(N, int):
            raise ValidationError(
                f"Grid size N must be integer, got {type(N).__name__}"
            )
            
        if N <= 0:
            raise ValidationError(
                f"Grid size N must be positive, got {N}"
            )
            
        if N % 2 != 0:
            raise ValidationError(
                f"Grid size N must be even, got {N}"
            )
            
        if N < 4:
            raise ValidationError(
                f"Grid size N must be at least 4, got {N}"
            )
...
def validate_timestep(dt: float, dt_min: float = 1e-10, 
                     dt_max: float = 1.0) -> float:
    ...
    if not isinstance(dt, (float, int, np.floating)):
        raise ValidationError(
            f"Timestep dt must be numeric, got {type(dt).__name__}"
        )
    ...
    if dt < dt_min:
        raise ValidationError(
            f"Timestep dt={dt} is below minimum {dt_min}"
        )
        
    if dt > dt_max:
        raise ValidationError(
            f"Timestep dt={dt} exceeds maximum {dt_max}"
        )
...
def validate_diffusivity(kappa: float, name: str = "kappa") -> float:
    ...
    if not isinstance(kappa, (float, int, np.floating)):
        raise ValidationError(
            f"{name} must be numeric, got {type(kappa).__name__}"
        )
    ...
    if kappa < 0:
        raise ValidationError(
            f"{name} must be non-negative, got {kappa}"
        )
        
    return kappa


def validate_array_shape(array: Union[np.ndarray, jnp.ndarray],
    ...
    if not hasattr(array, 'shape'):
        raise ValidationError(
            f"{name} must be an array, got {type(array).__name__}"
        )
...
def validate_complex_field(field_hat: Union[np.ndarray, jnp.ndarray],
                          name: str = "field_hat") -> None:
    ...
    if not hasattr(field_hat, 'dtype'):
        raise ValidationError(
            f"{name} must be an array, got {type(field_hat).__name__}"
        )
        
    if not np.issubdtype(field_hat.dtype, np.complexfloating):
        raise ValidationError(
            f"{name} must be complex dtype, got {field_hat.dtype}"
        )