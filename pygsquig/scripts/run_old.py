...
def main(config, device, checkpoint, output_dir, dry_run, log_level):
    ...
    while state["time"] < run_config.simulation.t_end and not shutdown_requested:
        ...
        if state["time"] - t_last_log >= run_config.simulation.log_interval:
            ...
            eta = elapsed * (run_config.simulation.t_end - state["time"]) / (state["time"] - t_last_log) if state["time"] > t_last_log else 0
            
            click.echo(f"t={state['time']:8.3f} | step={state['step']:6d} | "
        ...
        if state["time"] - t_last_output >= run_config.simulation.output_interval:
            ...
            for field in run_config.output.fields:
                if field == "theta":
                    ...
                    psi_hat = compute_streamfunction(state["theta_hat"], grid, run_config.solver.alpha)
            ...
            save_output(data, grid, state["time"], metadata, output_file, 
