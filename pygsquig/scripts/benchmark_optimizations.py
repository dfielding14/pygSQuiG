...
def create_plots(resolutions, results):
    ...
    ax1.loglog(N_array, np.array(orig_times)*1000, 'bo-', 
               label='Original', linewidth=2, markersize=8)
    ax1.loglog(N_array, np.array(opt_single)*1000, 'rs-', 
               label='Optimized (single)', linewidth=2, markersize=8)
    ax1.loglog(N_array, np.array(opt_multi)*1000, 'g^-', 
               label='Optimized (multistep)', linewidth=2, markersize=8)
    
    # Add O(N²) reference
    ax1.loglog(N_array, orig_times[0]*1000*(N_array/N_array[0])**2, 
    ...
    ax2.plot(N_array, speedup_single, 'rs-', label='Single step', 
             linewidth=2, markersize=8)
    ax2.plot(N_array, speedup_multi, 'g^-', label='Multistep', 
             linewidth=2, markersize=8)
