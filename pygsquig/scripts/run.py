...
from pygsquig.forcing import CombinedDamping, RingForcing
...
def create_source_from_config(source_config):
    ...
    if source_type == "exponential":
        ...
        return ChemicalReaction(rate=params.get("rate", 1.0), threshold=params.get("threshold"))
            ...
            if config.scalars and config.scalars.enabled:
                ...
                for species in config.scalars.species:
                    if species.initial_condition == "zero":
                        ...
                        scalar_init[species.name] = species.initial_params.get(
                            "amplitude", 1.0
                        ...
                        logger.warning(
                            f"Unknown initial condition '{species.initial_condition}' for {species.name}, using zero"
                        )
                        scalar_init[species.name] = jnp.zeros((grid.N, grid.N))

                state = solver.initialize(seed=seed, scalar_init=scalar_init)
                logger.info(
                    f"Initialized with random initial condition and {len(scalar_init)} scalar(s) (seed={seed})"
                )
            else:
...
def main(config, device, checkpoint, output_dir, dry_run, log_level):
    ...
    if run_config.forcing and run_config.forcing.type != "none":
        if run_config.forcing.type == "ring":
            ...
            logger.info(
                f"Initialized ring forcing: kf={run_config.forcing.kf}, ε={run_config.forcing.epsilon}"
            )
        else:
    ...
    logger.info(
        f"Starting simulation from t={initial_time:.2f} to t={run_config.simulation.t_end:.2f}"
    )
    ...
    while get_time(state) < run_config.simulation.t_end and not shutdown_requested:
        ...
        if current_time - t_last_log >= run_config.simulation.log_interval:
            ...
            logger.log_progress(current_time, get_step(state), dt, diagnostics, eta)

            # Save diagnostics
            save_diagnostics(diagnostics, current_time, dirs["diagnostics"] / "timeseries.h5")
        ...
        if get_time(state) - t_last_output >= run_config.simulation.output_interval:
            ...
            for field in run_config.output.fields:
                if field == "theta":
                    ...
                    theta_hat = (
                        state.theta_hat if hasattr(state, "theta_hat") else state["theta_hat"]
                    )
            ...
            save_output(
                data, grid, time_val, metadata, output_file, compress=run_config.output.compress
            )
