...
class PerformanceProfiler:
    ...
    def profile_operators(self):
        ...
        mean, std = self.time_operation(compute_velocity_from_theta, self.theta_hat, self.grid, 1.0)
    ...
    def create_performance_report(self):
        ...
        # Create plots
        self.plot_results(fft_results, operator_results, solver_results,
                         resolutions, step_times, fft_times, jacobian_times)
        ...
        print(f"\nMain bottlenecks:")
        print(f"  Jacobian:     {operator_results['jacobian']*1000:.2f} ms ({operator_results['jacobian']/rhs_time*100:.0f}% of RHS)")
        print(f"  FFT/IFFT:     {(fft_results['fft']+fft_results['ifft'])*1000:.2f} ms total")
        print(f"  Velocity:     {operator_results['velocity']*1000:.2f} ms")
        
    def plot_results(self, fft_results, operator_results, solver_results,
                     resolutions, step_times, fft_times, jacobian_times):
        ...
        ax.loglog(N_ref, step_times[0]*(N_ref/resolutions[0])**2, 'k--', alpha=0.5, label='O(N²)')
        ...
        sizes = [
            ...
            solver_results['rhs'] - operator_results['jacobian'] - operator_results['velocity'] - fft_results['fft'] - fft_results['ifft']
        ]
