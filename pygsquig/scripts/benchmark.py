...
def main():
    ...
    # Gradient
    times['gradient'] = benchmark_operator(
        "gradient", gradient, theta_hat, grid
    )
    
    # Velocity computation
    times['velocity'] = benchmark_operator(
        "compute_velocity_from_theta", 
        compute_velocity_from_theta, theta_hat, grid, alpha
    )
    
    # Jacobian (advection)
    times['jacobian'] = benchmark_operator(
        "jacobian", jacobian, theta, u, v, grid
    )
