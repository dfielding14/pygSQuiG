    ...
    if len(plot_quantities) > 0:
        ...
        if flux_data:
            fig = plot_time_series_multiplot(
                ...
                flux_data,
                labels={
                    "dissipation_rate": "Dissipation ε_ν",
...
def analyze_spectrum_evolution(
    ...
    if slopes:
        ...
        ax.axhline(-5 / 3, color="red", linestyle="--", alpha=0.7, label="SQG: -5/3")
        ax.set_xlabel("Time")
        ...
        fig.savefig(output_dir / "slope_evolution.png", dpi=PlotStyle.DPI, bbox_inches="tight")
        plt.close(fig)
...
def create_field_snapshots(
    ...
    for field_file in selected_files:
        ...
        if "velocity" in field_names:
            plot_velocity_fields(
                theta_hat, grid, alpha, output_path=output_dir / f"velocity_t{time:.1f}.png"
            )
...
def generate_analysis_report(
    ...
    report_lines.extend(["", "=" * 60, "Analysis completed successfully", "=" * 60])
...
@click.option(
    "--output-dir", "-o", type=click.Path(), help="Output directory for analysis results"
)
@click.option("--n-snapshots", "-n", default=5, help="Number of field snapshots to analyze")
@click.option("--create-animation", "-a", is_flag=True, help="Create animation of field evolution")
@click.option("--animation-field", default="theta", help="Field to animate")
@click.option(
    "--reference-slopes",
    "-r",
    multiple=True,
...
def main(
    simulation_dir, output_dir, n_snapshots, create_animation, animation_field, reference_slopes
):
    ...
    if not field_files:
        ...
        if create_animation:
            ...
            create_field_animation(field_files, animation_field, output_path=animation_path)
