...
def plot_spectrum(k, E_k, slope):
    ...
    ax.loglog(k_ref, E_ref_53, 'r--', linewidth=1.5, 
              label=r'$k^{-5/3}$', alpha=0.7)
    
    # k^(-3) reference (steep)
    E_ref_3 = E_k[30] * (k_ref / k_ref[0])**(-3)
    ax.loglog(k_ref, E_ref_3, 'g--', linewidth=1.5, 
              label=r'$k^{-3}$', alpha=0.7)
    
    # Measured slope
    k_fit = k[20:80]
    E_fit = E_k[30] * (k_fit / k_fit[0])**slope
    ax.loglog(k_fit, E_fit, 'k:', linewidth=2, 
              label=f'Measured: $k^{{{slope:.2f}}}$')
