...
class TaylorGreenForcing(DeterministicForcing):
    ...
    def __init__(self, amplitude: float = 1.0, k: int = 2, time_dependent: bool = False):
    ...
    def __call__(
        self, theta_hat: jax.Array, key: jax.random.PRNGKey, dt: float, grid: Grid
    ) -> jax.Array:
...
class KolmogorovForcing(DeterministicForcing):
    ...
    def __init__(self, amplitude: float = 1.0, k: int = 4, direction: Literal["x", "y"] = "y"):
...
class CheckerboardForcing(DeterministicForcing):
    ...
    def __init__(self, amplitude: float = 1.0, kx: int = 4, ky: Optional[int] = None):
...
class VortexPairForcing(DeterministicForcing):
    ...
    def __init__(self, vortices: list, amplitude_scale: float = 1.0):
        ...
        for v in vortices:
            ...
            self.vortices.append(
                {"x": float(x), "y": float(y), "circulation": float(circ), "radius": float(radius)}
            )
...
def make_taylor_green_forcing(
    amplitude: float = 1.0, k: int = 2, time_decay: bool = False
...
def make_kolmogorov_forcing(
    amplitude: float = 1.0, k: int = 4, direction: Literal["x", "y"] = "y"
) -> DeterministicForcing:
...
def make_oscillating_forcing(
    base_pattern: str = "taylor_green", frequency: float = 1.0, **kwargs
) -> DeterministicForcing:
    ...
    return TimeModulatedForcing(base_forcing=base, modulation="sine", frequency=frequency)
