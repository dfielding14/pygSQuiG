...
class WhiteNoiseForcing(StochasticForcing):
    ...
    def _normalize_energy_injection(
        self, 
        noise: Array, 
        grid: Grid, 
        dt: float
...
class StochasticVortexForcing(StochasticForcing):
    ...
    def __call__(
        ...
        if n_vortices > 0:
            ...
            # Random strengths
            strengths = self.amplitude * (1 + self.vortex_strength_std * 
                                         jax.random.normal(keys[2], (n_vortices,)))
            ...
            for i in range(n_vortices):
                ...
                r_squared = ((x - x_positions[i])**2 + 
                           (y - y_positions[i])**2)
                vortex = (strengths[i] * signs[i] * 
                         jnp.exp(-r_squared / (2 * radius**2)))
...
class OrnsteinUhlenbeckForcing(StochasticForcing):
    ...
    def __call__(
        ...
        if self._ou_state is None:
            ...
            self._ou_state = jax.random.normal(
                key_init, theta_hat.shape
            ) + 1j * jax.random.normal(key_init, theta_hat.shape)
        ...
        noise_amp = self.amplitude * jnp.sqrt(
            2 * dt / self.correlation_time * (1 - decay**2)
        )
        
        # Generate noise
        noise = (jax.random.normal(key_noise, theta_hat.shape) + 
                1j * jax.random.normal(key_noise, theta_hat.shape))
...
def create_combined_stochastic_forcing(
    forcings: list[StochasticForcing],
    ...
    class CombinedStochasticForcing(StochasticForcing):
        ...
        def __call__(
            ...
            for forcing, weight, subkey in zip(
                self.forcings, self.weights, keys
            ):
