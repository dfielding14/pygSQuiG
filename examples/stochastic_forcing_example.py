...
def example_white_noise():
    ...
    def animate(frame):
        ...
        im1 = axes[0].imshow(theta, cmap='RdBu_r', 
                            extent=[0, L, 0, L],
                            vmin=-3, vmax=3)
        ...
        axes[1].loglog(k_bins, k_bins**(-5/3), 'k--', 
                      alpha=0.5, label='k^{-5/3}')
        
        # Mark forcing range
        axes[1].axvspan(forcing.k_min, forcing.k_max, 
                       alpha=0.2, color='red', label='Forcing')
...
def example_colored_noise():
    ...
    for step in range(2000):
        ...
        if step % save_interval == 0:
            ...
            ax2.loglog(k_bins, k_bins**(-3), 'k--', 
                      alpha=0.5, label='k^{-3}')
            ax2.axvline(forcing.k_peak, color='red', 
                       linestyle=':', label='k_peak')
            ax2.set_xlabel('k')
            ax2.set_ylabel('E(k)')
...
def example_vortex_injection():
    ...
    for step in range(500):
        ...
        if step % 20 == 0:
            ...
            # Theta field
            axes[0, 0].clear()
            im = axes[0, 0].imshow(theta, cmap='RdBu_r', 
                                  extent=[0, L, 0, L],
                                  vmin=-5, vmax=5)
            ...
            axes[0, 1].clear()
            vort = -theta  # For SQG
            axes[0, 1].imshow(vort, cmap='RdBu_r', 
                            extent=[0, L, 0, L],
                            vmin=-5, vmax=5)
            ...
            axes[1, 0].loglog(k_bins, k_bins**(-5/3), 'k--', 
                            alpha=0.5, label='k^{-5/3}')
            ...
            axes[1, 1].clear()
            if vortex_times:
                axes[1, 1].scatter(vortex_times, 
                                 range(len(vortex_times)), 
                                 color='red', s=20)
...
def example_ou_forcing():
    ...
    forcing = OrnsteinUhlenbeckForcing(
        amplitude=0.3,
        correlation_time=0.5,  # Correlation time
        k_min=10.0,
        k_max=30.0
    )
    ...
    for step in range(n_steps):
        ...
        if len(forcing_history) > 1:
            corr = jnp.sum(
                forcing_history[0].conj() * forcing_history[-1]
            ).real
...
def example_combined_forcing():
    ...
    forcing1 = ColoredNoiseForcing(
        amplitude=0.2,
        spectral_slope=-2.0,
        k_peak=5.0,    # Large scale
        k_width=3.0
    )
    
    forcing2 = WhiteNoiseForcing(
        amplitude=0.3,
        k_min=20.0,    # Small scale
        k_max=40.0
    )
    
    forcing3 = StochasticVortexForcing(
        amplitude=1.0,
        vortex_size=0.1,
        injection_rate=0.5
    )
    ...
    combined_forcing = create_combined_stochastic_forcing(
        ...
        weights=[1.0, 1.0, 0.5]  # Relative weights
    )
    ...
    for step in range(1000):
        ...
        if step % 50 == 0:
            ...
            axes[0, 1].loglog(k_bins, k_bins**(-5/3), 'k--', 
                            alpha=0.5, label='k^{-5/3}')
            
            # Mark forcing ranges
            axes[0, 1].axvspan(3, 8, alpha=0.2, color='red', 
                             label='Large-scale forcing')
            axes[0, 1].axvspan(20, 40, alpha=0.2, color='green', 
                             label='Small-scale forcing')
            ...
            if solver.timestepper.dt_history:
                ...
                axes[1, 1].axhline(stats['dt_mean'], color='red', 
                                 linestyle='--', label=f'Mean: {stats["dt_mean"]:.3e}')
            axes[1, 1].set_xlabel('Time')
