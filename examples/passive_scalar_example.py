...
def main():
    ...
    passive_scalars = {
        ...
        'temperature': {
            'kappa': 0.01,
            'source': make_heating_source(
                x0=L/4, y0=L/2, radius=L/8, power=1.0
            )
        },
        # Chemical species with decay
        'chemical': {
            'kappa': 0.005,
            'source': make_cooling_source(decay_time=10.0)
        }
    }
    ...
    theta0 = np.real(np.fft.ifft2(
        np.fft.fft2(theta0) * (grid.k2 < 20**2)
    ))  # Low-pass filter
    ...
    scalar_init = {
        ...
        'temperature': np.exp(
            -((grid.x - 3*L/4)**2 + (grid.y - L/2)**2) / (L/8)**2
        ),
    ...
    while state.time < t_end:
        ...
        if state.time >= t_output:
            ...
            if t_output == 0 or state.time >= t_end - dt:
                ...
                im1 = axes[0, 0].imshow(
                    theta, cmap='RdBu_r', origin='lower',
                    extent=[0, L, 0, L]
                )
                ...
                # Passive dye
                im2 = axes[0, 1].imshow(
                    dye, cmap='viridis', origin='lower',
                    extent=[0, L, 0, L], vmin=0, vmax=1
                )
                axes[0, 1].set_title('Passive Dye')
                
                # Temperature
                im3 = axes[0, 2].imshow(
                    temp, cmap='hot', origin='lower',
                    extent=[0, L, 0, L], vmin=0
                )
