...
from pygsquig.core.multi_gpu import (
    MultiGPUSolver,
    ParallelConfig,
    ensemble_statistics
)
...
def example_2_ensemble_simulation():
    ...
    if results['states']:
        ...
        stats = ensemble_statistics(
            results['states'][-1],  # Final states
            grid,
            alpha=1.0
        )
        print(f"Final energy range: [{stats['energy_range'][0]:.6f}, {stats['energy_range'][1]:.6f}]")
...
def example_3_performance_comparison():
    ...
    if len(grid_sizes) > 1:
        ...
        for N in grid_sizes[1:]:
            ...
            print(f"  {base_N}→{N}: {scaling:.2f}x slower (expected {expected:.1f}x, efficiency {efficiency:.1%})")
...
def example_4_forced_turbulence_gpu():
    ...
    for step in range(n_steps):
        ...
        if step % output_interval == 0:
            ...
            print(f"Step {step:5d}: t={state['time']:6.3f}, E={energy:.4f}, "
